...
test()
```
通过这种 hack 的方式，我们就实现了对涉及 DOM 操作的组件的测试。我的 [vue-upload-imgs](https://github.com/dsh225/vue-upload-imgs) 库就是通过这种方式写的单元测试，有兴趣可以了解一下（测试文件放在 `test` 目录）。
...
```
## 小结
本章所有的测试用例都可以在我的 [github](https://github.com/dsh225/front-end-engineering-demo) 上找到，建议把项目克隆下来，亲自运行一遍。
