...
if (!commitRE.test(msg)) {
    console.error(`
        不合法的 commit 消息格式。
        请查看 git commit 提交规范：https://github.com/dsh225/Front-end-articles/blob/master/git%20commit%20style.md
    `)
...
另外，我提供了一个简单的工程化 [DEMO](https://github.com/dsh225/front-end-engineering-demo)。它包含了自动格式化代码和 git 验证，如果看完文章还是不知道如何配置，可以参考一下。
